...
struct Mylist{
   ...
   string chinese;
   void print(){
       cout<<"Hash num: "<<Hashnum<<" , setence: "<<chinese;
   }
   bool operator==(const Mylist &obj) {
    return Hashnum == obj.Hashnum;
}
...
class MyListCompare{
public:
    static bool lt(Mylist x,Mylist y){ return x.Hashnum < y.Hashnum;}
    static bool eq(Mylist x,Mylist y){ return x.Hashnum == y.Hashnum;}
    static bool gt(Mylist x,Mylist y){ return x.Hashnum > y.Hashnum;}
};
...
public:
	...
	bool isinHeap(Elem q);
	void buildheap() { for (int i=n/2-1; i>=0; i--) shiftdown(i); }
	void print() { for(int i=0; i<n; i++){ cout << " | "; heap[i].print();cout<<endl; }}
    Elem top() const { return heap[0]; }
    Elem extract(int i) const { return heap[i]; }
};

template<class Elem,class Comp>
bool MinHeap<Elem,Comp>::isinHeap(Elem q){
        for (int i=0;i<size;i++){
            if (heap[i]==q) return true;
        }
        return false;
    }
...
bool MinHeap<Elem, Comp>::insert(const Elem& val){
	...
	int curr = n++;
	heap[curr] = val;
	// Shift upward until curr's parent >= curr
